...
-- Multi threading (thread.c) {{{1

if not apr.thread_create then
  message "Not testing multi threading module (unavailable)\n"
else
  message "Testing multi threading module ..\n"
  ...
  local threadfile = os.tmpname() -- should't exist at this point
  ...
  local handle = assert(io.open(threadfile))
...
if apr.platform_get() == 'WIN32' then
  message "Not testing character encoding translation (broken on Windows)..\n"
else
  message "Testing character encoding translation ..\n"
...
writefile(xml_path, '<parent><child>text</child></parent>')
